<%-- 
    Document   : resultPagination
    Created on : 30 oct. 2013, 19:48:57
    Author     : juanlossio
--%>
...
<%
    ...
    int indice_begin = indice*120;
    int count_indice_begin = 0;
    ...
    String pagination = "<p class='pages'> "
            + "<small>Number of terms : " + num_term_by_val + " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Page "+(indice+1)+" of " + num_page + " &nbsp;</small>";
            //+ "<span>1</span> ";
            for(int j=0;j<num_page;j++){       
                pagination = pagination + "<a href='#' onclick='javascript:show_term_by_validation_by_interval("+j+")'>"+ (j+1) +"</a>";
            }
    ...
    if(term_validated == 1){
        ...
        if(term_validated == 2){
            ...
            while(count_terms<120 && i<len){
                if(list_candidat_terms.get(i).getIsTrueTerm()==1){
                    if(count_indice_begin+1>indice_begin){
                        html = html + "<tr>" + 
                                    "<td>"+(i+1)+"</td>";
